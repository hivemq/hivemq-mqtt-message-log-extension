...
* Add a configuration file named `config.xml` file to the extension `conf` folder.
...
NOTE: Release 1.2.0 deprecates the legacy configuration file `mqttMessageLog.properties`.
We encourage you to use `conf/config.xml` instead, as we will remove support for the legacy configuration in a future release.
...
There is an example file available in `conf/examples/` which removes the logging of PUBLISH messages.
